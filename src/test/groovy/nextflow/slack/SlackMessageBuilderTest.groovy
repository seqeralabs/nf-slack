...
class SlackMessageBuilderTest extends Specification {
    ...
    def 'should read watchUrl from platform workflowUrl metadata'() {
        given:
        def gcl = new GroovyClassLoader()
        def platformClass = gcl.parseClass('class FakePlatform { String workflowUrl = "https://cloud.seqera.io/orgs/myorg/workspaces/myws/watch/meta123" }')
        def platformConfig = new SlackConfig([
            webhook: 'https://hooks.slack.com/test',
            seqeraPlatform: [enabled: true]
        ])
        def mockMetadata = Mock(WorkflowMetadata)
        mockMetadata.scriptName >> 'test-workflow.nf'
        mockMetadata.platform >> platformClass.newInstance()
        def towerSession = Mock(Session)
        towerSession.config >> [:]
        towerSession.workflowMetadata >> mockMetadata
        towerSession.runName >> 'crazy_einstein'
        towerSession.uniqueId >> UUID.fromString('00000000-0000-0000-0000-000000000000')
        def builder = new SlackMessageBuilder(platformConfig, towerSession)

        expect:
        builder.getTowerClientWatchUrl() == 'https://cloud.seqera.io/orgs/myorg/workspaces/myws/watch/meta123'
    }
    ...
    def 'should include Seqera Platform button in progress update when watchUrl is available'() {
        ...
        when:
        def message = builder.buildProgressUpdateMessage(10, 5, 3, 1, 60000L, '1234567890.123456')
        def json = new JsonSlurper().parseText(message)
